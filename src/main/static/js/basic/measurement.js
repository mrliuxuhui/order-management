...
$(function () {
    ...
    $("#modal-add").on("show.bs.modal", function() {
        var measurementId = $(this).find("#measurementId").val();
        var numReg = new RegExp("^\\d+$");
        if(null!=measurementId&&numReg.test(measurementId)){
            $(this).find(".modal-title").text("更新计量单位");
            $("#data-form").attr("action","/admin/api/measurement/"+measurementId);
            $("#data-form").attr("method","put");

        }else{
            $(this).find(".modal-title").text("添加计量单位");
            $("#data-form").attr("action","/admin/api/measurement");
    ...
    $("#submitBtn").on("click",function(){
        ...
        if(validator.isValid()){
            ...
            //var createRex = new RegExp("measurement$");
            var updateRex = new RegExp("measurement/\\d+$");
            var method = "post";
    ...
    $("#delBtn").on("click",function(){
        ...
        var measurementIds = new Array();
        $.each(data, function (index, value) {
            measurementIds.push(value.id);
        });

        $.ajax({
            url:"/admin/api/measurement",
            type:"delete",
            dataType:"json",
            data:JSON.stringify(measurementIds),
            success:function(result){
                table.draw();
            }
        });
