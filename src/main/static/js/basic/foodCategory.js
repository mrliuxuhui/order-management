...
$(function () {
    ...
    $("#modal-add").on("show.bs.modal", function() {
        var foodCategoryId = $(this).find("#foodCategoryId").val();
        var numReg = new RegExp("^\\d+$");
        if(null!=foodCategoryId&&numReg.test(foodCategoryId)){
            $(this).find(".modal-title").text("更新菜品分类");
            $("#data-form").attr("action","/admin/api/foodCategory/"+foodCategoryId);
            $("#data-form").attr("method","put");

        }else{
            $(this).find(".modal-title").text("添加菜品分类");
            $("#data-form").attr("action","/admin/api/foodCategory");
    ...
    $("#submitBtn").on("click",function(){
        ...
        if(validator.isValid()){
            ...
            //var createRex = new RegExp("foodCategory$");
            var updateRex = new RegExp("foodCategory/\\d+$");
            var method = "post";
    ...
    $("#delBtn").on("click",function(){
        ...
        $.each(data, function (index, value) {
            foodCategoryIds.push(value.id);
        });

        $.ajax({
            url:"/admin/api/foodCategory",
            type:"delete",
            dataType:"json",
            data:JSON.stringify(foodCategoryIds),
            success:function(result){
                table.draw();
            }
        });
